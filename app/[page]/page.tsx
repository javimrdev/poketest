...
import { type PokemonList as PokemonListType, pokemonListSchema } from "@/logic/pokemon/schemas";

export default async function Page({ params }: { params: Promise<{ page: string }> }) {
  const { page } = await params;
  const isNaNPage = Number.isNaN(Number(page));

  if (isNaNPage || Number(page) < 1) {
    notFound();
  }

  let data: PokemonListType | undefined;
  try {
  ...
  if (!pokemonList.success) {
    ...
    return <ErrorPage title="Data Error" message="The Pokemon data received is invalid. Please try again later." />;
  }
  ...
  return (
    <div className="flex flex-col gap-4 w-full max-w-md mx-auto p-4">
      <h1 id="pokemon-list" className="text-2xl font-bold mb-2">
        Pokemons list
      </h1>
      <PokemonListWrapper>
